-- Payout verification is independent from business (company) verification.
-- Suppliers can save IBAN/BIC while business is under review; each axis locks only after submit + pending, or when verified.

alter table public.supplier_profiles
  add column if not exists payout_verification_status text
    check (
      payout_verification_status is null
      or payout_verification_status in ('pending', 'verified', 'rejected')
    ),
  add column if not exists payout_verification_submitted_at timestamptz;

comment on column public.supplier_profiles.payout_verification_status is
  'Traverion approval of payout (IBAN/BIC); independent from verification_status (business).';
comment on column public.supplier_profiles.payout_verification_submitted_at is
  'Set when supplier saves bank details for payout review; with pending/verified payout, locks payout fields.';

-- Grandfather: already business-approved with bank details on file → payout treated as approved.
update public.supplier_profiles
set
  payout_verification_status = 'verified',
  payout_verification_submitted_at = coalesce(verification_submitted_at, updated_at, created_at)
where verification_status = 'verified'
  and coalesce(trim(payout_iban), '') <> ''
  and coalesce(trim(payout_bic), '') <> ''
  and payout_verification_status is null;
...
comment on function public.supplier_profiles_enforce_verification_lock() is
  'Locks business columns vs payout columns independently; service_role JWT or postgres/supabase_admin bypasses.';
